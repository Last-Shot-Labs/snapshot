...
```ts
passkeyLogin.mutate({ passkeyToken, assertionResponse, redirectTo: '/dashboard' })
```
...
// In a component:
{login.isError && (
  <p>{formatAuthError(login.error, 'login')}</p>
)}
```
...
function ThreadFeed() {
  const { data: newThread, status } = useSseEvent<{ id: string; title: string }>(
    '/__sse/feed',
