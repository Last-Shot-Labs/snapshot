...
export function createMfaHooks({ api, storage, config, contract, pendingMfaChallengeAtom, onLoginSuccess }: MfaHooksOptions) {
  ...
  function useMfaVerifySetup() {
    return useMutation<MfaVerifySetupResponse, ApiError, MfaVerifySetupBody>({
      mutationFn: (body) => api.post<MfaVerifySetupResponse>(contract.endpoints.mfaVerifySetup, body),
    })
  }
  ...
  function useMfaRecoveryCodes() {
    return useMutation<MfaRecoveryCodesResponse, ApiError, MfaRecoveryCodesBody>({
      mutationFn: (body) => api.post<MfaRecoveryCodesResponse>(contract.endpoints.mfaRecoveryCodes, body),
    })
  }

  function useMfaEmailOtpEnable() {
    return useMutation<MfaEmailOtpEnableResponse, ApiError, void>({
      mutationFn: () => api.post<MfaEmailOtpEnableResponse>(contract.endpoints.mfaEmailOtpEnable, {}),
    })
  }

  function useMfaEmailOtpVerifySetup() {
    return useMutation<MfaVerifySetupResponse, ApiError, MfaEmailOtpVerifySetupBody>({
      mutationFn: (body) => api.post<MfaVerifySetupResponse>(contract.endpoints.mfaEmailOtpVerifySetup, body),
    })
  }

  function useMfaEmailOtpDisable() {
    return useMutation<{ message: string }, ApiError, MfaEmailOtpDisableBody>({
      mutationFn: (body) => api.delete<{ message: string }>(contract.endpoints.mfaEmailOtpDisable, body),
    })
  }
