...
interface AuthHooksOptions {
  ...
  config: Pick<SnapshotConfig, 'auth' | 'staleTime' | 'onUnauthenticated' | 'loginPath' | 'homePath' | 'mfaPath' | 'onLogoutSuccess'>
...
export function createAuthHooks({ api, storage, config, contract, pendingMfaChallengeAtom, onLoginSuccess, onLogoutSuccess }: AuthHooksOptions) {
  function useUser() {
