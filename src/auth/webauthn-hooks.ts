...
export function createWebAuthnHooks({ api, storage, config, contract, pendingMfaChallengeAtom, onLoginSuccess }: WebAuthnHooksOptions) {
  function useWebAuthnRegisterOptions() {
    return useMutation<WebAuthnRegisterOptionsResponse, ApiError, void>({
      mutationFn: () => api.post<WebAuthnRegisterOptionsResponse>(contract.endpoints.webauthnRegisterOptions, {}),
    })
  }
  ...
  function useWebAuthnRegister() {
    ...
    return useMutation<{ message: string }, ApiError, WebAuthnRegisterBody>({
      mutationFn: (body) => api.post<{ message: string }>(contract.endpoints.webauthnRegister, body),
      onSuccess: () => {
  ...
  function useWebAuthnCredentials() {
    const { data, isLoading, isError } = useQuery<{ credentials: WebAuthnCredential[] }, ApiError>({
      ...
      queryFn: () => api.get<{ credentials: WebAuthnCredential[] }>(contract.endpoints.webauthnCredentials),
    })
  ...
  function useWebAuthnRemoveCredential() {
    ...
    return useMutation<{ message: string }, ApiError, string>({
      mutationFn: (credentialId) => api.delete<{ message: string }>(contract.webauthnRemoveCredential(credentialId)),
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: WEBAUTHN_CREDENTIALS_KEY })
      },
    })
  ...
  function usePasskeyLoginOptions() {
    return useMutation<PasskeyLoginOptionsResponse, ApiError, PasskeyLoginOptionsBody>({
      mutationFn: (body) => api.post<PasskeyLoginOptionsResponse>(contract.endpoints.passkeyLoginOptions, body),
    })
  }
