...
const mockUseMutation = vi.fn(() => ({ mutate: mockMutate, mutateAsync: mockMutate }))
...
vi.mock('@tanstack/react-query', async (importOriginal) => {
  ...
  return {
    ...
    useQueryClient: () => ({ setQueryData: mockSetQueryData, clear: mockClear }),
...
describe('useLogout', () => {
  ...
  it('successful logout clears storage, refresh token, query cache, MFA atom, and navigates', async () => {
    const { useLogout } = createAuthHooks({ api, storage, config, pendingMfaChallengeAtom })
