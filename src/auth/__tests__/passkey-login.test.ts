...
describe('usePasskeyLogin', () => {
  ...
  it('fetches /auth/me (not fabricated user)', async () => {
    ...
    const result = await mutationFn({ passkeyToken: 'pt', assertionResponse: {} })
  ...
  it('redirectTo override works', async () => {
    setup()
    const onSuccess = capturedMutationConfig.onSuccess as Function
    await onSuccess(mockUser, { passkeyToken: 'pt', assertionResponse: {}, redirectTo: '/dashboard' })
    expect(mockNavigate).toHaveBeenCalledWith({ to: '/dashboard' })
  })

  it('MFA challenge path matches useLogin', async () => {
    const mfaResponse = { mfaRequired: true, mfaToken: 'mfa-tok', mfaMethods: ['totp'] }
    ...
    expect(mockSetAtom).toHaveBeenCalledWith({ mfaToken: 'mfa-tok', mfaMethods: ['totp'] })
  ...
  it('redirectTo is stripped from request body', async () => {
    setup()
    const mutationFn = capturedMutationConfig.mutationFn as Function
    await mutationFn({ passkeyToken: 'pt', assertionResponse: {}, redirectTo: '/somewhere' })
