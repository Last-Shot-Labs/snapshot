...
export function createWebhookHooks({ api, queryClient: _qc }: { api: ApiClient; queryClient: QueryClient }) {
  ...
  function useUpdateWebhookEndpoint() {
    const queryClient = useQueryClient()
    return useMutation<WebhookEndpointResponse, ApiError, { endpointId: string } & UpdateWebhookEndpointBody>({
      mutationFn: ({ endpointId, ...body }) =>
        api.patch<WebhookEndpointResponse>(`/webhooks/endpoints/${endpointId}`, body),
      onSuccess: (_data, { endpointId }) => {
        void queryClient.invalidateQueries({ queryKey: keys.endpoints() })
        void queryClient.invalidateQueries({ queryKey: keys.endpoint(endpointId) })
      },
    })
  }
  ...
  function useTestWebhookEndpoint() {
    ...
    return useMutation<void, ApiError, { endpointId: string } & TestWebhookBody>({
      ...
      onSuccess: (_data, { endpointId }) => {
        void queryClient.invalidateQueries({ queryKey: keys.deliveries(endpointId) })
      },
    })
  }
