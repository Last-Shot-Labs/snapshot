...
import type { SnapshotConfig, SnapshotInstance, CommunityNotification, UseCommunityNotificationsOpts, UseCommunityNotificationsResult, SseHookResult, SseEventHookResult } from './types'
...
>(
  ...
  const authErrorsConfig = (config as unknown as Record<string, unknown>).authErrors as { verbose?: boolean } | undefined
  ...
  function useCommunityNotifications(opts?: UseCommunityNotificationsOpts): UseCommunityNotificationsResult {
    ...
    const fetchNotifications = useCallback(async (): Promise<CommunityNotification[]> => {
      try {
        const result = await api.get<{ items: CommunityNotification[]; nextCursor?: string }>(apiBase)
    ...
    useEffect(() => {
      ...
      return () => { sseManager.off('community:notification.created', handler) }
    }, [fetchNotifications]) // eslint-disable-line react-hooks/exhaustive-deps
   ...
   */
  function onSseEvent(endpoint: string, event: string, handler: (payload: unknown) => void): () => void {
    const entry = sseRegistry.get(endpoint)
    if (!entry) return () => {}
  ...
  // ── Routing ─────────────────────────────────────────────────────────────────
  const { protectedBeforeLoad, guestBeforeLoad } = createLoaders(config, api, contract)
