...
export function useDataSource<T = unknown>(
  ...
  const fromRefs = useMemo(() => {
    ...
    return Object.entries(params)
      .filter((entry): entry is [string, { from: string }] =>
  ...
  // Build stable query key from method, path, and resolved params
...
export function useMutationSource<TData = unknown, TBody = unknown>(
  ...
  const mutation = useMutation<TData, ApiError, TBody>({
    mutationFn: (body: TBody) => {
      switch (method) {
        case 'POST': return api.post<TData>(path, body)
        case 'PUT': return api.put<TData>(path, body)
        case 'PATCH': return api.patch<TData>(path, body)
        case 'DELETE': return api.delete<TData>(path, body)
        default: return api.post<TData>(path, body)
      }
    },
    onSuccess: () => {
